function [p_wt,p_pv,p_load,x,UB,p_g,p_ch,p_dis,p_buy,p_sell] = DOWN_(ee_bat_int,p_wt_int,p_pv_int,p_g_int,LB,yita)
%% 本函数通过对偶原理计算 折现成本+投资成本+购售电成本和燃料成本 +运维成本（求最大值）
%% 1.设参
canshu;
...
C = [C, P+Q1'*lam1-Q1'*lam11+Q2'*lam2-Q2'*lam21+Q3'*lam3+Q4'*lam4-Q4'*lam41+Q5'*lam5-Q5'*lam51+Q6'*lam6<=M.*beta7,x<=M.*(1-beta7)];
 ...
 %% 目标函数 春季典型日
 %购售电成本+燃料成本+运维成本
obj_o = sum(sum(repmat(price,1,4).*(p_buy(:,:)-p_sell(:,:)))+c_fuel*sum(p_g(:,1))+...%购售电成本+燃料成本+运维成本
