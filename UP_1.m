function [yita,LB,ee_bat_int, p_wt_int,p_pv_int,p_g_int]=UP_1(rp,rbat,rPV,rWT,rG)
%% 本函数计算初始折现成本+投资成本+购售电成本和燃料成本 +运维成本（求最小值）
%% 1.设参
canshu;
%% 2.设决策变量
%储能出力
...
C = [C, 0 <= p_dis,p_dis <= y1];    
C = [C, y1 <= p_bat_int];%储能的功率上限
C = [C, y1 >= p_bat_int-1000*uu_bat];
C = [C, y1 >= 20*(1 - uu_bat),y1 <= 1000*(1 - uu_bat)];
 
C = [C, 0 <= p_ch,p_ch <= y2];    
C = [C, y2 <= p_bat_int];
C = [C, y2 >= p_bat_int - 1000*(1 - uu_bat)];
C = [C, y2 >= 20*uu_bat,y2 <= 1000*uu_bat];
...
P = [price' -price' 0.659*ones(1,24) 0.009*ones(1,48) price' -price' 0.659*ones(1,24) 0.009*ones(1,48) price' -price' 0.659*ones(1,24) 0.009*ones(1,48) price' -price' 0.659*ones(1,24) 0.009*ones(1,48)]';
B = repmat([0.0296*ones(1,24) 0.0096*ones(1,24) zeros(1,24)]',1,4);
...
C = [C, yita >= sum(sum(repmat(price,1,4).*(p_buy(:,:) - p_sell(:,:))) + c_fuel*sum(p_g(:,1))...%购售电成本和燃料成本+运维成本
